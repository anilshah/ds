...
public class Payment extends JInternalFrame {
    ...
    public static String rpayNo,rpassName,ramount,rmode,rdate,rby;
    public Payment() {
        ...
        button1.addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent e) {
                ...
                if (combo1.getSelectedItem() == null) {
                    JOptionPane.showMessageDialog(null, "All Passenger have paid", "Error",JOptionPane.DEFAULT_OPTION);
                    return;
                }
                ...
                if (combo8.getSelectedItem() == null) {
                    JOptionPane.showMessageDialog(null, "The Passenger has not been booked", "ERROR",JOptionPane.DEFAULT_OPTION);
                    return;
                }
                try {
                    Statement statement = DBConnection.getDBConnection().createStatement();
                    {
                        String temp = "INSERT INTO Payment (Payment_No, Pass_No, Pass_Name, Payment_Mode, Date_Payment,Amount_Paid,Received_By) VALUES ('" +text1.getText() + "', '" +combo1.getSelectedItem() + "', '" +combo2.getSelectedItem() + "', '" +combo4.getSelectedItem() + "', '" +p_date.getText() + "', '" +combo8.getSelectedItem() + "', '" +combo3.getSelectedItem() + "')";
                        int result = statement.executeUpdate(temp);

                   rpayNo=text1.getText();
                   rpassName=(String)combo2.getSelectedItem();
                   ramount=(String)combo8.getSelectedItem();
                   rmode=(String)combo4.getSelectedItem();
                   rdate=p_date.getText();
                   rby=(String)combo3.getSelectedItem();

      


                        JOptionPane.showMessageDialog(null, "Passenger Account updated", "Updated",JOptionPane.DEFAULT_OPTION);
                ...
                try {
                    Statement statement = DBConnection.getDBConnection().createStatement();
                    {
                        String temp = "UPDATE Passenger SET Pay_Status='Paid'" +"WHERE Pass_NO LIKE  '" + combo1.getSelectedItem() + "'";
                        int result = statement.executeUpdate(temp);
...
//             combo2.removeItem(combo2.getSelectedItem());
   ...
   private void generator() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Payment_No FROM Payment order by Payment_No");
            text1.setText("1000");
            while (rst.next()) {
                String s;
    ...
    private void setCombo() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Emp.empNo, Emp.Sname, Emp.Fname, Emp.Lname, Emp.Designation FROM Emp WHERE Emp.Designation='Booking Clerk'");
            while (rst.next()) {
    ...
    private void setcbr() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT * FROM Passenger where Booked_Status='Booked' and Pay_Status='Not_Paid'");
            while (rst.next()) {
    ...
    private void setamount() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Amount FROM BOOKING  where Pass_No='" + combo1.getSelectedItem() + "'");
            while (rst.next()) {
    ...
    public void display(ResultSet rs) {
        try {
            ...
            if (recordNumber) {
                ...
                System.out.println(payNo+"   "+pasNo+"    "+ pasName+"     "+mode+"    "+dt+"    "+amount+"    "+rev);
...
//                combo1.addItem(pasNo);
//                combo2.addItem(pasName);
////                combo4.setSelectedItem(mode);
////                p_date.setText(dt);
//                combo8.addItem(amount);
////                combo3.setSelectedItem(rev);
