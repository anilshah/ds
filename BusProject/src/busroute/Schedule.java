...
//		jPanel4.add(Enable);
    ...
    private void setCbx() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Bus_RegNo,BusNo FROM Buses order by BusNo");
    ...
    private void setrt() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Route_No,RouteName FROM Route ");
    ...
    private void setCombo() {
        ...
        dr = "Driver";
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Emp.empNo, Emp.Sname, Emp.Fname, Emp.Lname, Emp.Designation FROM Emp WHERE Emp.Designation='Driver'");
            while (rst.next()) {
                cboDriverNo.addItem(rst.getString(1));
                cboDriverName.addItem(rst.getString(3));
            }
        } catch (Exception n) {
            n.printStackTrace();
        }
            ...
            if (recordfound) {
                ...
                try {
                    if (cboRouteNo.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please choose route number", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }
                    if (cboRouteName.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please Choose RouteName", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }
                    if (cboBusNo.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please choose bus number", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }
                    if (cboRegNo.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please choose registration number", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }
                    if (cboDriverNo.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please choose driver number", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }
                    if (cboDriverName.getSelectedItem() == ("Select")) {
                        JOptionPane.showMessageDialog(null, "please choose DriverName", "INFORMATION",JOptionPane.DEFAULT_OPTION);
                        return;
                    }

                    Statement statement = DBConnection.getDBConnection().createStatement();
                    {
                        String temp = "INSERT INTO Schedules (Bus_No,Bus_RegNo, Route_No, Route_Name,empNo,Driver_Name,Date_Scheduled,Trip_No,Dept_Time) VALUES ('" +cboBusNo.getSelectedItem() + "', '" +cboRegNo.getSelectedItem() + "', '" +cboRouteNo.getSelectedItem() + "', '" +cboRouteName.getSelectedItem() + "', '" +cboDriverNo.getSelectedItem() + "', '" +cboDriverName.getSelectedItem() + "', '" +s_date.getText() + "', '" +cboTrip.getSelectedItem() + "', '" +txtDepTime.getText() + "')";
                        int result = statement.executeUpdate(temp);
                        String temp2 = "INSERT INTO Validator(Bus_No,DriverNo,RouteNo,Date_Schedule,Trip_No)VALUES('" +cboBusNo.getSelectedItem() + "', '" +cboDriverNo.getSelectedItem() + "', '" +cboRouteNo.getSelectedItem() + "' ,'" +s_date.getText() + "' ,'" +cboTrip.getSelectedItem() + "')";

                        int results = statement.executeUpdate(temp2);

                        JOptionPane.showMessageDialog(null, "Scheduling Succesfully done", "Sucess??",JOptionPane.DEFAULT_OPTION);
                    }
